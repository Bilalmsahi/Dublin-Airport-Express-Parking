...
import { RESCHEDULE_DAY_RATE, getCancellationPolicy, isDepartureLocked, isReturnLocked } from "../utils/bookingRules";
...
const CustomerDashboard = () => {
  ...
  const handleCancel = async (bookingId, type) => {
    ...
    try {
      ...
      if (data.outcome === "no_refund") {
        alert("Your booking was cancelled. As it was cancelled within 72 hours of departure, no refund applies.");
      } else if (data.outcome === "points") {
        // Bonus points are credited server-side — just sync the new balance
        const updatedUser = {
          ...user,
          bonus_points: Number(data.bonus_points ?? user.bonus_points ?? 0),
        };
        setUser(updatedUser);
        setBookingData((prev) => ({
            ...prev,
        ...
        alert("Your booking was cancelled. €15 cancellation fee has been deducted. Credit points have been added to your account.");
      } else {
        ...
        prev.map(b =>
          b.id === bookingId
            ? {
                ...
                departure_time,
                return_time,
                status: { ...b.status, name: b.status?.name === "Started" ? "Started" : "Rescheduled" },
              }
            : b
        )
      );
  ...
  const handleRescheduleSubmit = async (data) => {
    ...
    // Only require payment if status is Confirmed, Rescheduled or Started and extra days > 0.
    // Reductions change nothing — no refund, no credit points, price unchanged.
    if (
      (booking.status?.name === "Confirmed" || booking.status?.name === "Rescheduled" || booking.status?.name === "Started") &&
      extraDays > 0
    ) {
      const extraAmount = RESCHEDULE_DAY_RATE * extraDays;
      ...
      setPayPopup({
        ...
        newReturnTime: data.return_time,
  ...
  return (
    <>
      <Helmet>
        <title>My Bookings - Dublin Airport Express Parking</title>
        <meta
          name="description"
          content="View, reschedule, or cancel your Dublin Airport Express Parking bookings. Check payment status and manage your reservations easily."
        />
      </Helmet>
      {/* Banner */}
      <div
              ...
              <tbody>
                {sortedBookings.map((b) => (
                  <tr key={b.id} style={{ verticalAlign: "middle" }}>
                    <td>
                      <span style={{ fontWeight: 600, color: "#75C24B", fontSize: "0.9rem" }}>
                        {b.booking_id || b.id}
                      </span>
                    </td>
                    <td>
                    ...
                    <td>
                        {b.discounted_price && (Number(b.discounted_price) < Number(b.total_price) && Number(b.discounted_price) > 0) ? (
                            ...
                            <span style={{ fontWeight: 700, color: "#073E46" }}>
                            €{b.total_price || "0.00"}
                            </span>
                        )}
                    </td>
                      ...
                      <div className="d-flex gap-2 flex-wrap justify-content-center">
                        <button
                          ...
                          style={{
                            ...
                            opacity:
                              !(b.status?.name === "Rescheduled" || b.status?.name === "Confirmed" || b.status?.name === "Pending" || b.status?.name === "Started") ||
                              actionLoading
                                ? 0.5
                                : 1,
                            cursor:
    ...
    <ReschedulePopup
        ...
        lockDeparture={reschedulePopup.booking ? isDepartureLocked(reschedulePopup.booking) : false}
        lockReturn={reschedulePopup.booking ? isReturnLocked(reschedulePopup.booking) : false}
        dayRate={RESCHEDULE_DAY_RATE}
      />
      ...
      <PayPopup
        ...
        newDepartureTime={payPopup.newDepartureTime || null}
        newReturnTime={payPopup.newReturnTime || null}
      />
    <style>
      {`
      .minimal-table {
    ...
    <CancelBookingModal
      ...
      allowPoints={true}
      refundEligible={cancelModal.booking ? getCancellationPolicy(cancelModal.booking).refundEligible : true}
      loading={actionLoading}
      onConfirm={async (type) => {
        const bookingId = cancelModal.booking.id;
        setCancelModal({ open: false, booking: null });
        await handleCancel(bookingId, type === "points" ? "points" : "refund");
      }}
    />
  </>
);
